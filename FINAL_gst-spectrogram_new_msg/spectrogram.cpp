...
static GstFlowReturn spectrogram_transform_ip(GstBaseTransform *trans, GstBuffer *buffer) {
	...
	while (filter->audio_buffer.size() >= SAMPLE_LEN) {
        ...
        if (image_r.size() == SPEC_SHAPE[0] && image_r[0].size() == SPEC_SHAPE[1] &&
            ...
            // print some info to compare with receive end
            std::cout << "\033[1m\033[35mSpectrogram Plugin sends one message...\n\033[0m";
            std::cout << "image_r: ";
            for(int i = 50; i < 60; i++){
            ...
            std::cout << std::endl;
            std::cout << "\033[1m\033[33mSpectrogram Plugin finished sending one message...\n\033[0m";
